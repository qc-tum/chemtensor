...
char* test_su2_tensor_norm2()
{
	...
	for (int ndim_auxiliary = 0; ndim_auxiliary < 2; ndim_auxiliary++)
	{
		...
		struct su2_tensor t;
		{
			...
			struct su2_tree_node j5f = { .i_ax = 5, .c = { &j3,  &j2  } };
			struct su2_tree_node j5s = { .i_ax = 5, .c = { &j0,  &j6  } };
			...
			const ct_long dim_degen1[] = { 1, 0, 4    };
			const ct_long dim_degen2[] = { 0, 5, 0, 2 };
			const ct_long dim_degen3[] = { 0, 0, 2    };
			const ct_long dim_degen4[] = { 0, 4, 0, 3 };  // only used if ndim_auxiliary == 0
			...
			su2_tensor_delete_charge_sector_by_index(&t, 1);
			...
			scomplex alpha = 0.01;
			su2_tensor_fill_random_normal(&alpha, numeric_zero(t.dtype), &rng_state, &t);
		}

		const double nrm = su2_tensor_norm2(&t);

		// dense tensor representation
		struct dense_tensor t_dns;
		su2_to_dense_tensor(&t, &t_dns);

		const double nrm_ref = dense_tensor_norm2(&t_dns);

		// compare
		if (fabs(nrm - nrm_ref) / nrm_ref > 5e-5) {
			return "logical norm of SU(2) symmetric tensor does not match reference value";
		}
...
char* test_su2_tensor_fuse_axes_add_auxiliary()
{
	...
	for (int f14 = 0; f14 < 2; f14++)  // whether to flip axes 1 <-> 4 in the tree
	{
		for (int f24 = 0; f24 < 2; f24++)  // whether to flip axes 2 <-> 4 in the tree
		{
			// construct an SU(2) tensor 't'
			struct su2_tensor t;
			{
				...
				{
					// flip axes 1 <-> 4
					const int axis_map[9] = { 0, 4, 2, 3, 1, 5, 6, 7, 8 };
					su2_fuse_split_tree_update_axes_indices(&tree, axis_map);
				}

				if (f24 == 1)
				{
					// flip axes 2 <-> 4
					const int axis_map[9] = { 0, 1, 4, 3, 2, 5, 6, 7, 8 };
					su2_fuse_split_tree_update_axes_indices(&tree, axis_map);
				}
			...
			const int i_ax_0 = 1;
			const int i_ax_1 = (f24 == 0 ? 4 : 2);
			struct su2_tensor r;
			su2_tensor_fuse_axes_add_auxiliary(&t, i_ax_0, i_ax_1, &r);
			...
			if (r.tree.tree_split->c[1]->c[1 - f14]->i_ax != i_ax_0) {
				return "first logical axis after fusing two axes not found in fusion-splitting tree";
			}
			if (r.tree.tree_split->c[1]->c[f14]->i_ax != r.ndim_logical) {
			...
			struct su2_tensor t2;
			{
				...
				su2_tensor_split_axis_remove_auxiliary(&r, i_ax_0, i_ax_1, outer_irreps, dim_degen, &t2);
			}

			if (!su2_tensor_is_consistent(&t2)) {
				return "internal consistency check for SU(2) tensor failed";
			}
			if (!su2_fuse_split_tree_equal_topology(&t2.tree, &t.tree)) {
				return "fusion-splitting tree of SU(2) tensor after axis fusion and splitting and adding and removing an auxiliary axis must have same topology as the original tree";
			}
			...
			su2_to_dense_tensor(&t, &t_dns);
			if (!dense_su2_tensor_allclose(&t_dns, &t2, 1e-13)) {
				return "tensor after axis fusion and splitting and adding and removing an auxiliary axis should agree with original tensor";
			}
			delete_dense_tensor(&t_dns);
...
char* test_su2_tensor_slice()
{
	...
	struct su2_tensor t;
	{
		...
		struct su2_tree_node j3  = { .i_ax = 3, .c = { NULL, NULL } };
		struct su2_tree_node j4  = { .i_ax = 4, .c = { NULL, NULL } };
		struct su2_tree_node j6  = { .i_ax = 6, .c = { &j0,  &j3  } };
		struct su2_tree_node j5f = { .i_ax = 5, .c = { &j1,  &j2  } };
		struct su2_tree_node j5s = { .i_ax = 5, .c = { &j4,  &j6  } };
		...
		// degeneracy dimensions, indexed by 'j' quantum numbers
		//                         j:  0  1  2  3  4
		const ct_long dim_degen0[] = { 0, 0, 0, 2    };
		const ct_long dim_degen1[] = { 3, 0, 4       };
		const ct_long dim_degen2[] = { 0, 6, 0, 4    };
		const ct_long dim_degen3[] = { 2, 0, 3, 0, 5 };
		const ct_long* dim_degen[] = {
			dim_degen0,
			dim_degen1,
			dim_degen2,
	...
	const int i_ax_slice = 3;
	const ct_long ind_slice[] = { 1, 5, 7, 8 };

	struct su2_tensor r;
	su2_tensor_slice(&t, i_ax_slice, ind_slice, ARRLEN(ind_slice), &r);

	if (!su2_tensor_is_consistent(&r)) {
		return "internal consistency check for SU(2) tensor failed";
	}

	for (int k = 0; k < r.outer_irreps[i_ax_slice].num; k++) {
		if (r.outer_irreps[i_ax_slice].jlist[k] == 2) {
			return "quantum number 'j == 2' should be absent in SU(2) tensor after slicing";
		}
	}
	...
	{
		const qnumber j = t.outer_irreps[i_ax_slice].jlist[k];
		assert(t.dim_degen[i_ax_slice][j] > 0);
		sector_offsets[j] = d;
		d += t.dim_degen[i_ax_slice][j] * (j + 1);
	}
	assert(d == t_dns.dim[i_ax_slice]);

	ct_long* ind_slice_dns = ct_malloc(d * sizeof(ct_long));  // upper bound on required memory
	ct_long nind_dns = 0;
	for (ct_long i = 0; i < (ct_long)ARRLEN(ind_slice); i++)
	{
		// find 'j' quantum number sector of current index
		ct_long b = 0;
		int k;
		for (k = 0; k < t.outer_irreps[i_ax_slice].num; k++)
		{
			const qnumber j = t.outer_irreps[i_ax_slice].jlist[k];
			if (b <= ind_slice[i] && ind_slice[i] < b + t.dim_degen[i_ax_slice][j]) {
				break;
			}
			b += t.dim_degen[i_ax_slice][j];
		}
		assert(k < t.outer_irreps[i_ax_slice].num);
		const qnumber j = t.outer_irreps[i_ax_slice].jlist[k];

		const ct_long ind_loc = ind_slice[i] - b;
		assert(ind_loc < t.dim_degen[i_ax_slice][j]);
		for (ct_long l = ind_loc * (j + 1); l < (ind_loc + 1) * (j + 1); l++) {
			ind_slice_dns[nind_dns++] = sector_offsets[j] + l;
		}
	}

	struct dense_tensor r_ref;
	dense_tensor_slice(&t_dns, i_ax_slice, ind_slice_dns, nind_dns, &r_ref);

	// compare
	if (!dense_su2_tensor_allclose(&r_ref, &r, 0.)) {
		return "SU(2) tensor after axis slicing does not match reference";
	}

	ct_free(ind_slice_dns);
...
char* test_su2_tensor_slice_scale()
{
	struct rng_state rng_state;
	...
	struct su2_tensor t;
	{
		...
		struct su2_tree_node j4  = { .i_ax = 4, .c = { NULL, NULL } };
		struct su2_tree_node j5  = { .i_ax = 5, .c = { NULL, NULL } };
		struct su2_tree_node j8  = { .i_ax = 8, .c = { &j1,  &j4  } };
		struct su2_tree_node j7  = { .i_ax = 7, .c = { &j8,  &j2  } };
		struct su2_tree_node j6f = { .i_ax = 6, .c = { &j5,  &j7  } };
		struct su2_tree_node j6s = { .i_ax = 6, .c = { &j3,  &j0  } };
		...
		//                         j:  0  1  2  3
		const ct_long dim_degen0[] = { 5, 0, 2    };
		const ct_long dim_degen1[] = { 0, 3       };
		const ct_long dim_degen2[] = { 0, 4, 0, 6 };
		const ct_long dim_degen3[] = { 0, 1, 0, 7 };
		const ct_long* dim_degen[] = {
			dim_degen0,
			dim_degen1,
			dim_degen2,
		...
		// delete some charge sectors
		su2_tensor_delete_charge_sector_by_index(&t, 0);
		su2_tensor_delete_charge_sector_by_index(&t, 5);
	...
	const int i_ax_slice = 2;
	const ct_long ind_slice[] = { 5, 7, 9 };
	// scaling factors
	double fac[ARRLEN(ind_slice)];
	for (ct_long i = 0; i < (ct_long)ARRLEN(fac); i++) {
		fac[i] = randn(&rng_state);
	}
	struct dense_tensor s;
	const ct_long sdim[1] = { ARRLEN(fac) };
	allocate_dense_tensor(CT_DOUBLE_REAL, 1, sdim, &s);
	memcpy(s.data, fac, sizeof(fac));

	struct su2_tensor r;
	su2_tensor_slice_scale(&t, i_ax_slice, ind_slice, &s, &r);
	...
	for (int k = 0; k < r.outer_irreps[i_ax_slice].num; k++) {
		if (r.outer_irreps[i_ax_slice].jlist[k] == 1) {
			return "quantum number 'j == 1' should be absent in SU(2) tensor after slicing";
		}
	}
	...
	ct_long* ind_slice_dns = ct_malloc(d * sizeof(ct_long));  // upper bound on required memory
	double*  fac_slice_dns = ct_malloc(d * sizeof(double));
	ct_long nind_dns = 0;
	for (ct_long i = 0; i < (ct_long)ARRLEN(ind_slice); i++)
	{
		// find 'j' quantum number sector of current index
		...
		for (ct_long l = ind_loc * (j + 1); l < (ind_loc + 1) * (j + 1); l++) {
			ind_slice_dns[nind_dns] = sector_offsets[j] + l;
			fac_slice_dns[nind_dns] = fac[i];
			nind_dns++;
		}
	}

	struct dense_tensor s_slice_dns;
	const ct_long ssd_dim[1] = { nind_dns };
	allocate_dense_tensor(s.dtype, 1, ssd_dim, &s_slice_dns);
	memcpy(s_slice_dns.data, fac_slice_dns, nind_dns * sizeof(double));
	ct_free(fac_slice_dns);

	struct dense_tensor r_ref;
	dense_tensor_slice_scale(&t_dns, i_ax_slice, ind_slice_dns, &s_slice_dns, &r_ref);

	// compare
	if (!dense_su2_tensor_allclose(&r_ref, &r, 1e-14)) {
		return "SU(2) tensor after axis slicing and scaling does not match reference";
	}

	delete_dense_tensor(&s_slice_dns);
	ct_free(ind_slice_dns);
	ct_free(sector_offsets);
	delete_dense_tensor(&r_ref);
	delete_dense_tensor(&t_dns);
...
char* test_su2_dense_tensor_dot()
{
	struct rng_state rng_state;
	seed_rng_state(48, &rng_state);

	// data types
	for (enum numeric_type dtype = 0; dtype < CT_NUM_NUMERIC_TYPES; dtype++)
	{
		...
		struct su2_tensor s;
		{
			...
			struct su2_tree_node j5  = { .i_ax = 5, .c = { &j2,  &j4  } };
			struct su2_tree_node j6f = { .i_ax = 6, .c = { &j5,  &j0  } };
			struct su2_tree_node j6s = { .i_ax = 6, .c = { &j3,  &j1  } };
			...
			// degeneracy dimensions, indexed by 'j' quantum numbers
			//                         j:  0  1  2  3  4  5
			const ct_long dim_degen0[] = { 9, 0, 3, 0, 5    };
			const ct_long dim_degen1[] = { 0, 0, 0, 7, 0, 2 };
			const ct_long dim_degen2[] = { 4, 0, 0, 0, 1    };
			const ct_long dim_degen3[] = { 0, 0, 6, 0, 2    };
			const ct_long* dim_degen[] = {
				dim_degen0,
				dim_degen1,
				dim_degen2,
			...
			allocate_su2_tensor(dtype, ndim_logical, ndim_auxiliary, &tree, outer_irreps, dim_degen, &s);

			// delete some charge sectors
			su2_tensor_delete_charge_sector_by_index(&s, 0);
			su2_tensor_delete_charge_sector_by_index(&s, 5);

			// fill degeneracy tensors with random entries
			dcomplex alpha;
			numeric_from_double(0.05, s.dtype, &alpha);
			su2_tensor_fill_random_normal(&alpha, numeric_zero(s.dtype), &rng_state, &s);
		...
		for (int ndim_mult = 2; ndim_mult <= 4; ndim_mult++)
		{
			for (enum tensor_axis_range axrange_s = 0; axrange_s < TENSOR_AXIS_RANGE_NUM; axrange_s++)
			{
				...
				{
					// construct a dense tensor 't'
					struct dense_tensor t;
					{
						const int ndim_t = 5;
						ct_long dim_t[5] = { 5, 9, 4, 7, 3 };
						const int i_ax_offset_s = (axrange_s == TENSOR_AXIS_RANGE_LEADING ? 0 : s.ndim_logical - ndim_mult);
						const int i_ax_offset_t = (axrange_t == TENSOR_AXIS_RANGE_LEADING ? 0 : ndim_t - ndim_mult);
						// overwrite to-be contracted dimensions with the ones from 's'
						for (int i = 0; i < ndim_mult; i++)
						{
							dim_t[i_ax_offset_t + i] = su2_tensor_dim_logical_axis(&s, i_ax_offset_s + i);
						}

						allocate_dense_tensor(dtype, ndim_t, dim_t, &t);

						dcomplex alpha;
						numeric_from_double(0.05, t.dtype, &alpha);
						dense_tensor_fill_random_normal(&alpha, numeric_zero(t.dtype), &rng_state, &t);
					}

					struct dense_tensor r;
					su2_dense_tensor_dot(&s, axrange_s, &t, axrange_t, ndim_mult, &r);

					if (dense_tensor_norm2(&r) == 0) {
						return "expecting a non-zero tensor resulting from dot product of an SU(2) symmetric and a dense tensor";
					}

					// reference calculation
					struct dense_tensor r_ref;
					dense_tensor_dot(&s_dns, axrange_s, &t, axrange_t, ndim_mult, &r_ref);

					// compare
					if (!dense_tensor_allclose(&r, &r_ref, tol)) {
						return "dot product of an SU(2) symmetric and a dense tensor does not agree with reference tensor";
					}
		...
		for (int lfs = 0; lfs < 2; lfs++)
		{
			...
			for (int iaux = 0; iaux < 2; iaux++)
			{
				...
				struct su2_tensor a;
				{
					...
					// outer (logical and auxiliary) 'j' quantum numbers
					qnumber j0list[] = { 0, 1, 2, 3, 5, 7 };  // note: mixed half-integer and integer quantum numbers
					qnumber j1list[] = { 2, 4, 5, 6, 7 };     // note: mixed half-integer and integer quantum numbers
					qnumber j2list[] = { 0 };  // auxiliary
					const struct su2_irreducible_list outer_irreps[3] = {
						{ .jlist = j0list, .num = ARRLEN(j0list) },
						{ .jlist = j1list, .num = ARRLEN(j1list) },
					...
					// degeneracy dimensions, indexed by 'j' quantum numbers
					//                         j:  0  1  2  3  4  5  6  7
					const ct_long dim_degen0[] = { 3, 5, 7, 8, 0, 4, 0, 6 };
					const ct_long dim_degen1[] = { 0, 0, 9, 0, 1, 3, 2, 4 };
					const ct_long* dim_degen[] = {
						dim_degen0,
						dim_degen1,
					};

					allocate_su2_tensor(CT_SINGLE_COMPLEX, 2, 1, &tree, outer_irreps, dim_degen, &a);
					delete_su2_fuse_split_tree(&tree);
				...
				if (su2_tensor_rq(&a, mode, &r, &q) < 0) {
					return "SU(2) symmetric RQ decomposition failed internally";
				}
				...
				if (q.ndim_logical != 2) {
					return "expecting two logical dimensions for Q tensor from SU(2) symmetric RQ decomposition";
				}
				if (r.ndim_logical != 2) {
					return "expecting two logical dimensions for R tensor from SU(2) symmetric RQ decomposition";
				}
				if (q.ndim_auxiliary != a.ndim_auxiliary) {
					return "expecting same number of auxiliary dimensions in Q tensor from SU(2) symmetric RQ decomposition as in original A tensor";
				}
				if (su2_tensor_dim_logical_axis(&q, 1) != su2_tensor_dim_logical_axis(&a, 1)) {
					return "trailing dimension of Q from SU(2) symmetric RQ decomposition must be equal to the trailing dimension of A";
				}
				if (su2_tensor_dim_logical_axis(&r, 0) != su2_tensor_dim_logical_axis(&a, 0)) {
					return "leading dimension of R from SU(2) symmetric RQ decomposition must be equal to the leading dimension of A";
				}
				if (mode == QR_COMPLETE) {
					// 'q' must be a logical square matrix
					if (su2_tensor_dim_logical_axis(&q, 0) != su2_tensor_dim_logical_axis(&q, 1)) {
						return "Q from SU(2) symmetric RQ decomposition must be a logical square matrix for the \"complete\" mode";
					}
				}
				// 'q' must be an isometry
				if (!su2_tensor_is_isometry(&q, 1e-5, true)) {
					return "Q tensor from SU(2) symmetric RQ decomposition is not an isometry";
				}

				// degeneracy tensors in 'r' are upper triangular, but not neccessarily the overall logical matrix

				// logical matrix product 'r q' must be equal to 'a'
				struct su2_tensor rq;
				if (r.tree.tree_fuse->i_ax == 1)
				{
					const int i_ax_r[1] = { 1 };
					const int i_ax_q[1] = { 0 };
					su2_tensor_contract_simple(&r, i_ax_r, &q, i_ax_q, 1, &rq);
				}
				else
				{
					// include auxiliary axis for contraction
					const int i_ax_r[2] = { 1, 2 };
					const int i_ax_q[2] = { 0, 2 };
					// add a dummy auxiliary axis to 'q' to ensure that contracted tensor has three axes
					su2_tensor_add_auxiliary_axis(&q, 1, false);
					su2_tensor_contract_simple(&r, i_ax_r, &q, i_ax_q, 2, &rq);
				}
				// comparing dense tensor representations since number or locations of auxiliary axes might be different
				struct dense_tensor rq_dns;
				su2_to_dense_tensor(&rq, &rq_dns);
				if (!dense_su2_tensor_allclose(&rq_dns, &a, 1e-5)) {
					return "logical matrix product R Q from SU(2) symmetric RQ decomposition is not equal to original A tensor";
				}
				delete_dense_tensor(&rq_dns);
				delete_su2_tensor(&rq);
