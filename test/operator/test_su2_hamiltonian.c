...
char* test_fermi_hubbard_1d_su2_mpo()
{
	hid_t file = H5Fopen("../test/operator/data/test_fermi_hubbard_1d_su2_mpo.hdf5", H5F_ACC_RDONLY, H5P_DEFAULT);
	if (file < 0) {
		return "'H5Fopen' in test_fermi_hubbard_1d_su2_mpo failed";
	}

	// number of lattice sites
	int nsites;
	if (read_hdf5_attribute(file, "nsites", H5T_NATIVE_INT, &nsites) < 0) {
		return "reading number of lattice sites 'nsites' from disk failed";
	}

	// Hamiltonian parameters
	double t, u, mu;
	if (read_hdf5_attribute(file, "t", H5T_NATIVE_DOUBLE, &t) < 0) {
		return "reading Hamiltonian parameter 't' from disk failed";
	}
	if (read_hdf5_attribute(file, "u", H5T_NATIVE_DOUBLE, &u) < 0) {
		return "reading Hamiltonian parameter 'u' from disk failed";
	}
	if (read_hdf5_attribute(file, "mu", H5T_NATIVE_DOUBLE, &mu) < 0) {
		return "reading Hamiltonian parameter 'mu' from disk failed";
	}

	struct su2_mpo fermi_hubbard_1d_mpo;
	construct_fermi_hubbard_1d_su2_mpo(nsites, t, u, mu, &fermi_hubbard_1d_mpo);
	if (!su2_mpo_is_consistent(&fermi_hubbard_1d_mpo)) {
		return "internal consistency check for SU(2) symmetric Fermi-Hubbard Hamiltonian MPO failed";
	}

	// convert to a matrix
	struct dense_tensor fermi_hubbard_1d_mat;
	{
		struct su2_tensor fermi_hubbard_1d_tensor;
		su2_mpo_to_tensor(&fermi_hubbard_1d_mpo, &fermi_hubbard_1d_tensor);
		if (fermi_hubbard_1d_tensor.ndim_logical != 2 * nsites + 2) {
			return "expecting the SU(2) tensor representation of an SU(2) MPO to have logical degree '2 * nsites + 2'";
		}
		su2_to_dense_tensor(&fermi_hubbard_1d_tensor, &fermi_hubbard_1d_mat);
		delete_su2_tensor(&fermi_hubbard_1d_tensor);
		// reshape into a matrix
		const ct_long hspace_dim = ipow(4, nsites);
		const ct_long dim[2] = { hspace_dim, hspace_dim };
		reshape_dense_tensor(2, dim, &fermi_hubbard_1d_mat);
	}
	...
	{
		hsize_t dim_hsize[2];
		if (get_hdf5_dataset_dims(file, "fermi_hubbard_1d_mat", dim_hsize) < 0) {
			return "obtaining dimensions of reference Hamiltonian failed";
		}
		const ct_long dim[2] = { dim_hsize[0], dim_hsize[1] };
		allocate_dense_tensor(CT_DOUBLE_REAL, 2, dim, &fermi_hubbard_1d_mat_ref);
		// read values from disk
	...
	// compare
	if (!dense_tensor_allclose(&fermi_hubbard_1d_mat, &fermi_hubbard_1d_mat_ref, 1e-13)) {
		return "matrix representation of SU(2) symmetric Fermi-Hubbard Hamiltonian based on MPO form does not match reference";
	}

	// clean up
	delete_dense_tensor(&fermi_hubbard_1d_mat_ref);
	delete_dense_tensor(&fermi_hubbard_1d_mat);
	delete_su2_mpo(&fermi_hubbard_1d_mpo);
