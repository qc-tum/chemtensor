...
void bug_flow_update_basis(const struct ttno* hamiltonian, const int i_site, const int i_parent,
	const struct block_sparse_tensor* avg_bonds, const struct block_sparse_tensor* env_parent, const struct block_sparse_tensor* s0,
	const void* prefactor, const double dt, struct ttns* state,
	struct block_sparse_tensor* avg_bonds_augmented, struct block_sparse_tensor* augment_maps);

	
void bug_flow_update_connecting_tensor(const struct block_sparse_tensor* op, const struct block_sparse_tensor* c0,
	const struct abstract_graph* topology, const int i_site, const int i_parent,
	const struct block_sparse_tensor* avg_bonds_augmented, const struct block_sparse_tensor* env_parent,
