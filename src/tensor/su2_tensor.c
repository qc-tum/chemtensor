...
///
/// \brief Slice a logical axis of the tensor by selecting "degeneracy" indices 'ind' along this axis,
/// and pointwise-multiply the entries along this axis by the vector 's'.
///
/// Indices 'ind' cannot be duplicate and must be sorted.
/// 'j' quantum numbers along 'i_ax' for which the new degeneracy dimension would be zero are removed.
/// Memory will be allocated for 'r'.
///
void su2_tensor_slice_scale(const struct su2_tensor* restrict t, const int i_ax, const ct_long* ind, const struct dense_tensor* restrict s, struct su2_tensor* restrict r)
{
	assert(0 <= i_ax && i_ax < t->ndim_logical);
	assert(s->ndim == 1);
	...
	// partition scaling factors by 'j' quantum numbers
	struct dense_tensor* s_sectors = ct_malloc((j_max + 1) * sizeof(struct dense_tensor));
	const size_t s_dtype_size = sizeof_numeric_type(s->dtype);
	ct_long offset = 0;
	for (int k = 0; k < t->outer_irreps[i_ax].num; k++)
	{
		const qnumber j = t->outer_irreps[i_ax].jlist[k];
		if (dim_degen_r_i_ax[j] > 0)
		{
			const ct_long dim[1] = { dim_degen_r_i_ax[j] };
			allocate_dense_tensor(s->dtype, 1, dim, &s_sectors[j]);

			// casting to int8_t* to ensure that pointer arithmetic is performed in terms of bytes
			memcpy(s_sectors[j].data, (int8_t*)s->data + offset * s_dtype_size, dim_degen_r_i_ax[j] * s_dtype_size);

			offset += dim_degen_r_i_ax[j];
		}
	}
	assert(offset == nind);
	...
	for (ct_long cr = 0; cr < r->charge_sectors.nsec; cr++)
	{
		...
		const qnumber j = jlist[i_ax];
		assert(r->dim_degen[i_ax][j] == dim_degen_r_i_ax[j]);
		assert(r->dim_degen[i_ax][j] == s_sectors[j].dim[0]);
		dense_tensor_slice_scale(dt, i_ax, ind_sectors[j], &s_sectors[j], r->degensors[cr]);
	}
	...
	for (int k = 0; k < t->outer_irreps[i_ax].num; k++)
	{
		const qnumber j = t->outer_irreps[i_ax].jlist[k];
		if (dim_degen_r_i_ax[j] > 0) {
			delete_dense_tensor(&s_sectors[j]);
		}
		ct_free(ind_sectors[j]);
	}
	ct_free(s_sectors);
