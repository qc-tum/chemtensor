...
def recoupling_matrix(ja: int, jb: int, jc: int, js: int):
    ...
    return [[(sqrt((je + 1) * (jf + 1)) * racah(S(ja)/2, S(jb)/2, S(js)/2, S(jc)/2, S(je)/2, S(jf)/2)).evalf(n=17, strict=True)
             for jf in jf_range]
             for je in je_range]
        ...
        for jb in range(num_jb):
            for jc in range(num_jc):
                for js in js_range(ja, jb, jc):
                    ...
                    assert dim[0] == dim[1]
                    expr = (f"static const double su2_recoupling_{ja}_{jb}_{jc}_{js}[{dim[0]}][{dim[1]}] = "
